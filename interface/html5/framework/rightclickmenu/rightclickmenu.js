    ...
    var // currently active contextMenu trigger
        ...
        handle = {
            ...
            contextmenu: function (e) {
				...
				//FairnessTNA specific customizations to prevent right-click context menu from appearing within an expanded dropdown/combo/awesomeboxes
				// Prior to this, some users would right-click on an expanded awesomebox item, then click "Delete" thinking it deletes just that one item, but it would deleted the entire record.
				// As well disable right-click on collapsed dropdown/combo/awesomeboxes
				if( $(e.target).parents('.a-dropdown').length || $(e.target).parents('.a-combobox').length || $(e.target).hasClass('a-dropdown') || $(e.target).hasClass('a-combobox') ) {
					   e.preventDefault();
					   document.elementFromPoint( e.pageX, e.pageY ).click();
					   return;
				}

				//FairnessTNA specific customizations to prevent right-click context menu from appearing on input/textarea, so user can right-click copy&paste.
				if ( e.target.nodeName.toLowerCase() === 'input' || e.target.nodeName.toLowerCase() === 'textarea' || e.target.nodeName.toLowerCase() === 'select' ) {
					   return;
				}

		...
		switch ( this.current_step ) {
			case 'license':
				...
				if ( stepData['install_mode'] ) {
					...
					var license_html = $.i18n._( 'The installer has already been run, as a safety measure it has been disabled from running again. If you are absolutely sure you want to run it again, or upgrade your system, please go to your settings.ini.php file and set "installer_enabled" to "TRUE". The line should look like' ) + ':';
					...
					license_html = license_html + '<br>';
					license_html = license_html + '<br>';
					license_html = license_html + $.i18n._( 'For help, please visit' ) + ' <a href="https://github.com/aydancoskun/FairnessTNA">https://github.com/aydancoskun/FairnessTNA</a> ';
			...
			case 'requirements':
				...
				if ( stepData.check_all_requirements != 0 ) {
					...
					if (stepData.tt_product_edition > 10) {
						step_title_htm = step_title_htm
							...
							+ '<a href="https://github.com/aydancoskun/FairnessTNA" target="_blank">' + $.i18n._('FairnessTNA support') + '</a>';
					} else if (stepData.tt_product_edition == 10) {

						step_title_htm = step_title_htm
							+ '<p style="background-color: #FFFF00">'
							+ $.i18n._('For installation assistance, please join our community') + ' '
							+ '<a href="https://github.com/aydancoskun/FairnessTNA" target="_blank">' + $.i18n._('forums') + '</a>'
							+ ' ' + $.i18n._('or contact a FairnessTNA support expert for') + ' '
							+ '<a href="https://github.com/aydancoskun/FairnessTNA" target="_blank">' + $.i18n._('Implementation Support Services') + '</a></p>';
					}
				...
				// fairnesstna version, php version.
				var requirements_column1 = requirements.find( '.first-column' );
				requirements_column1.empty();

				form_item_input = Global.loadWidgetByName( FormItemType.TEXT );
				form_item_input.TText( { field: 'fairnesstna_version' } );
				this.addEditFieldToColumn( $.i18n._( 'FairnessTNA Version' ), form_item_input, requirements_column1, '' );
				...
				if ( stepData.check_all_requirements == 0 ) {
					...
					this.addEditFieldToColumn( $.i18n._( 'Writable' ) + ' ' + $.i18n._( stepData.application_name ) + ' ' + $.i18n._( 'Configuration File' ) + '<br>' + '(settings.ini.php)', form_item_input, requirements_column2, '', null, true, true );
				...
				columns_html = columns_html + '(settings.ini.php)';
		...
		switch ( this.current_step ) {
			...
			case 'requirements':
				for ( var key in widgets ) {
					...
					switch ( key ) {
						case 'fairnesstna_version':
							if ( stepData[key].check_fairnesstna_version == 0 ) {
								widget.html( $.i18n._( 'OK' ) + '(v' + stepData[key].current_fairnesstna_version + ')' );
							} else if ( stepData[key].check_fairnesstna_version == 1 ) {
								widget.html( $.i18n._( 'Unable to Check Latest Version' ) );
								widget.addClass( 'dataWarning' );
							} else if ( stepData[key].check_fairnesstna_version == 2 ) {
								widget.html(
										$.i18n._( 'A Newer Version of FairnessTNA is Available.' ) + ' '
										+ '<a href="https://github.com/aydancoskun/FairnessTNA">' + $.i18n._( 'Download' ) + ' v' + stepData[key].latest_fairnesstna_version + ' ' + $.i18n._( 'Now' ) + '</a>' );
								widget.addClass( 'dataWarning' );
							}
							break;
						case 'php_version':
						...
						case 'database_engine':
							if ( stepData[key] == 0 ) {
								...
								widget.html( $.i18n._( 'MySQL is not supported. (Database type must be "postgres" in settings.ini.php)' ) );
								widget.addClass( 'dataError' );
							}
							break;
						case 'bcmath':
						...
						case 'base_url':
							if ( stepData[key].check_base_url == 0 ) {
								...
								widget.html( key + ' ' + $.i18n._( 'in' ) + ' settings.ini.php ' + $.i18n._( 'is incorrect, perhaps it should be' ) + ' ' + stepData[key].recommended_base_url + ' ' + $.i18n._( 'instead' ) );
								widget.addClass( 'dataError' );
							}
							break;
						case 'base_dir':

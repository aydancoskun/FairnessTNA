...
class RecurringScheduleFactory extends Factory
{
    ...
    public function recalculateRecurringSchedules($user_id, $start_date, $end_date)
    {
    ...
    public function clearRecurringSchedulesFromRecurringScheduleControl($id, $start_date, $end_date)
    {
        global $amf_message_id;
    ...
    public function addRecurringSchedulesFromRecurringScheduleControl($company_id, $id, $start_date, $end_date)
    {
    ...
    public function addScheduleFromRecurringSchedule($company_obj, $start_date, $end_date)
    {
        $current_epoch = time();
            ...
            foreach ($rslf as $rs_obj) {
                ...
                if ($sf->isValid()) {
                    ...
                    if ($schedule_result == true
                        ...
                        and ($rs_obj->getAutoFill() == true)
                        and $rs_obj->getStatus() == 10
                    ) {
                        ScheduleFactory::addPunchFromScheduleObject($rs_obj);
                    } else {
                        Debug::text('  Not processing AutoFill... AutoFill: ' . (int)$rs_obj->getAutoFill() . ' Status: ' . $rs_obj->getStatus(), __FILE__, __LINE__, __METHOD__, 10);
                    }
                } else {
                    $sf->FailTransaction();
                    $sf->CommitTransaction();

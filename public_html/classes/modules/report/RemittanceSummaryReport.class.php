...
class RemittanceSummaryReport extends Report
{
    ...
    public function _pdf_Header()
    {
        if ($this->pdf->getPage() == 1) {
            ...
            if (is_array($columns) and count($columns) > 0) {
                ...
                $this->pdf->writeHTMLcell(100, 5, (($this->pdf->getPageWidth() - 100) / 2), $this->pdf->getY(), '<a href="https://apps.cra-arc.gc.ca/ebci/fppp/mypymnt/pub/ntr.action">PAY ONLINE NOW</>', 1, 0, false, true, 'C');
    ...
    public function _html_Header()
    {
        ...
        if (is_array($columns) and count($columns) > 0) {
            ...
            $this->html .= '<tr><th class="pay-online" colspan="' . count($columns) . '"><a href="https://apps.cra-arc.gc.ca/ebci/fppp/mypymnt/pub/ntr.action" target="_blank">PAY ONLINE NOW</a></th></tr>';
            $this->html .= '</thead>';
            $this->html .= '</table>';
        }

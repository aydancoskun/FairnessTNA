...
class bitmask
{
     ...
     */
    public function unset_bit($bitnum)
    {
     ...
     * @access private
     */
    public function _set_mask_array()
    {
     ...
     */
    public function add_assoc($assoc_bits)
    {
     ...
     */
    public function set_bit($bitnum)
    {
        if (!$this->bit_isset($bitnum)) {
            $masknum = (($bitnum - ($bitnum % 32)) / 32);
            $masks = explode('-', $this->forward_mask);
     ...
     */
    public function assoc_get($all_values = true, $force = false)
    {
        if ((count($this->assoc_keys) < count($this->mask_array)) && !$force) {
            die('More bits than array keys');
        }
        if ($all_values) {
     ...
     */
    public function reverse_mask($mask)
    {

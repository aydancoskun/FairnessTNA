...
class Config_Container
{
     ...
     * @return object  reference to new item or Pear_Error
     */
    public function &createItem($type, $name, $content, $attributes = null, $where = 'bottom', $target = null)
    {
     ...
     */
    public function &addItem(&$item, $where = 'bottom', $target = null)
    {
     ...
     * @return object  reference to new item or Pear_Error
     */
    public function &createSection($name, $attributes = null, $where = 'bottom', $target = null)
    {
     ...
     * @access   public
     */
    public function &searchPath($args)
    {
        if ($this->type != 'section') {
     ...
     * @return int  returns int or null if root object
     */
    public function getItemPosition($byType = true)
    {
        if (is_object($this->parent)) {
            $pchildren =& $this->parent->children;
            for ($i = 0, $count = count($pchildren); $i < $count; $i++) {
     ...
     */
    public function &setDirective($name, $content, $index = -1)
    {
     ...
     * @return object  reference to new item or Pear_Error
     */
    public function &createDirective($name, $content, $attributes = null, $where = 'bottom', $target = null)
    {
     ...
     */
    public function writeDatasrc($datasrc, $configType, $options = array())
    {
     ...
     * @return   mixed   true on success or PEAR_ERROR
     */
    public function toString($configType, $options = array())
    {
        $configType = strtolower($configType);
        if (!isset($GLOBALS['CONFIG_TYPES'][$configType])) {

...
class Net_DIME_Message extends PEAR
{
     ...
     */
    public function Net_DIME_Message($stream = null, $record_size = 4096, $debug = false)
    {
        $this->stream = $stream;
     ...
     *
     */
    public function sendData(&$data, $typestr = '', $id = null, $type = NET_DIME_TYPE_UNKNOWN)
    {
        $len = strlen($data);
    ...
    public function startChunk(&$data, $typestr = '', $id = null, $type = NET_DIME_TYPE_UNKNOWN)
    {
        $this->me = 0;
        $this->cf = 1;
        $this->type = $type;
    ...
    public function _makeRecord(&$data, $typestr = '', $id = null, $type = NET_DIME_TYPE_UNKNOWN)
    {
     ...
     *
     */
    public function sendFile($filename, $typestr = '', $id = null, $type = NET_DIME_TYPE_UNKNOWN)
    {
        $f = fopen($filename, "rb");
        if ($f) {
     ...
     *
     */
    public function encodeData($data, $typestr = '', $id = null, $type = NET_DIME_TYPE_UNKNOWN)
    {
        $len = strlen($data);
        $resp = '';
        if ($len > $this->record_size) {
     ...
     *
     */
    public function encodeFile($filename, $typestr = '', $id = null, $type = NET_DIME_TYPE_UNKNOWN)
    {
        $f = fopen($filename, "rb");
        if ($f) {
            if ($data = fread($f, $this->record_size)) {

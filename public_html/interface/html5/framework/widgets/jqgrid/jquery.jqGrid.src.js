    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            if (this.p.multiselect) {
                this.p.colNames.unshift("<input role='checkbox' id='cb_" + this.p.id + "' class='cbox-header' type='checkbox'/>");
                this.p.colModel.unshift({
                    name: 'cb',
                    width: $.jgrid.cellWidth() ? ts.p.multiselectWidth + ts.p.cellLayout : ts.p.multiselectWidth,
                    sortable: false,
                    resizable: false,
                    hidedlg: true,
                    search: false,
                ...
                if (cSel === true) {
                    ...
                    if (ts.p.cellEdit === true) {
                        ...
                        if (ts.p.multiselect && ts.p.multiboxonly) {
                            if (scb || e.metaKey || e.ctrlKey) { //Kevin: add ctrl key multi selection support
                                $(ts).jqGrid("setSelection", ri, true, e);
                        ...
                        if (nm !== 'cb' && nm !== 'subgrid' && this.editable === true && nm !== 'rn') {
                            if (ind === false) {
                                ...
                                if (nm == obj.p.ExpandColumn && obj.p.treeGrid === true) {
                                    ...
                                    try {
                                        tmp = $.unformat.call(obj, $("td:eq(" + i + ")", obj.rows[ind]), {
                                            rowId: rowid,
                                            colModel: this
                                        }, i);
                                    } catch (_) {
...
function tableToGrid(selector, options) {
    jQuery(selector).each(function () {
        ...
        // Build up the column_model and the data
        var colModel = [];
        var colNames = [];

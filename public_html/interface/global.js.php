...
function calendar_setup(input_field, button_id, show_time) {
    if (show_time == true) {
        date_format = "<?php if (isset($current_user_prefs)) {
            echo $current_user_prefs->getJSDateFormat() . ' ' . $current_user_prefs->getJSTimeFormat();
        } else {
            echo '';
        } ?>";
    } else {
        date_format = "<?php if (isset($current_user_prefs)) {
            echo $current_user_prefs->getJSDateFormat();
        } else {
            echo '';
        } ?>";
    }
    ...
    Calendar.setup(
        {
            ...
            showsTime: show_time,
            electric: false,
            firstDay: <?php if (isset($current_user_prefs)) {
            echo (int)$current_user_prefs->getStartWeekDay();
        } else {
            echo '0';
        } ?>,
            weekNumbers: false,
...
//Namespace for FairnessTNA objects
var FAIRNESS = function () {
    return {
        form: {},
        searchForm: {},
        invoice: {},
...
FAIRNESS.punch = function () {
    return {
        ...
        getJobOptionsCallBack: function (result) {
            ...
            FAIRNESS.punch.getJobManualId();
            FAIRNESS.punch.selectJobOption();
        },
        getJobItemOptionsCallBack: function (result) {
            if (document.getElementById('job_item_id') && document.getElementById('selected_job_item')) {
                ...
                FAIRNESS.punch.getJobItemManualId();
                FAIRNESS.punch.selectJobItemOption();
            }
        },
        showJob: function () {
...
FAIRNESS.searchForm = function () {
    ...
    return {
        ...
        toggleTabBlock: function (tabObjectID, contentObjectID, groupID) {
            ...
            if (show_block == true) {
                FAIRNESS.searchForm.showTab(tabObjectID, contentObjectID, groupID);
                FAIRNESS.searchForm.toggleTabBlockImage('up');
            } else {
                FAIRNESS.searchForm.deactivateTabs();
                FAIRNESS.searchForm.hideTabContent();
                FAIRNESS.searchForm.toggleTabBlockImage('down');
            }
        },
            ...
            if (document.getElementById) {
                if (tabs[groupID] != null) {
                    if (document.getElementById('tab_' + objectID)) {
                        FAIRNESS.searchForm.deactivateTabs(groupID);
        ...
        hideTabContent: function (groupID) {
            ...
            if (document.getElementById) {
                if (tabs[groupID] != null) {
                    ...
                    FAIRNESS.searchForm.hideTabGlobalContent(groupID);
                }
            }
        },
            ...
            if (document.getElementById) {
                if (tabs[groupID] != null) {
                    if (document.getElementById(objectID)) {
                        FAIRNESS.searchForm.hideTabContent(groupID);
                    ...
                    if (rows[row].id == row_prefix + 'all' || rows[row].id == row_prefix + tabObjectID) {
                        ...
                        if (tabObjectID == 'basic_search') {
                            ...
                            for (var tag = 0; tag < tags.length; tag++) {
                                FAIRNESS.searchForm.clearFormElement(tags[tag].name);
                            }
        ...
        onLoadShowTab: function () {
            if (document.getElementById('selected_tab')) {
                ...
                if (selected_tab != '') {
                    FAIRNESS.searchForm.showTab(selected_tab);
                }
            }
        },
        showTab: function (tabObjectID, contentObjectID, groupID) {
            ...
            if (document.getElementById) {
                if (tabs[groupID] != null) {
                    //Activate tab
                    FAIRNESS.searchForm.activateTab(tabObjectID, groupID);

                    //Show tab content
                    FAIRNESS.searchForm.showTabContent(contentObjectID, groupID);

                    //Show tab global content
                    FAIRNESS.searchForm.showTabGlobalContent(contentObjectID, groupID);

                    //Filter content within tab
                    FAIRNESS.searchForm.filterTabContent(tabObjectID);
                }
            }
        },
        clearForm: function () {
            for (num in document.search_form.elements) {
                if (document.search_form.elements[num]) {
                    ...
                    if (typeof element.type != 'undefined' && typeof element.name != 'undefined'
                        ...
                        FAIRNESS.searchForm.clearFormElement(element.name);
                    } else {
            ...
            for (num in document.search_form.elements) {
                if (document.search_form.elements[num]) {
                    ...
                    if (typeof element.type != 'undefined' && typeof element.name != 'undefined'
                        ...
                        FAIRNESS.searchForm.copyElementByName(element.name, dst_name);
                    }
                }
            }

...
<table border="0" cellpadding="0" cellspacing="0">
    <tr>
        ...
        <td align="left">
            ...
            {else}
                <a href="{urlbuilder script=$smarty.server.SCRIPT_NAME values="page=1"}" class="pagingLink"><img
                            src="{$IMAGES_URL}/paging_first.gif"> {t}Start{/t}</a>
                &nbsp;
                <a href="{urlbuilder script=$smarty.server.SCRIPT_NAME values="page=$previous_page"}"
                   class="pagingLink"><img src="{$IMAGES_URL}/paging_prev.gif"> {t}Previous{/t}</a>
            {/if}
        </td>
        <td width="10"><br></td>
        <td>
            ...
            <span class="textPaging">
                ...
                {section name="pages" start=1 loop=$last_page_number+1}
                    ...
                    {if $smarty.section.pages.index == $paging_data.current_page}
                        ...
                        <a href="{urlbuilder script=$smarty.server.SCRIPT_NAME values="page=$page_number"}"
                           class="pagingLink">{$smarty.section.pages.index}</a>
                    {/if}
                    {/if}
				{sectionelse}
        ...
        <td align="right">
            ...
            {else}
                <a href="{urlbuilder script=$smarty.server.SCRIPT_NAME values="page=$next_page"}"
                   class="pagingLink">{t}Next{/t} <img src="{$IMAGES_URL}/paging_next.gif"></a>
                &nbsp;
                <a href="{urlbuilder script=$smarty.server.SCRIPT_NAME values="page=$last_page_number"}"
                   class="pagingLink">{t}End{/t} <img src="{$IMAGES_URL}/paging_last.gif"></a>
            {/if}
        </td>
    </tr>
</table>

...
<div id="container">
    ...
    <div id="rowHeaderLogin"><a href="http://{$ORGANIZATION_URL}"><img
                    src="{if $exception == 'dberror'}{$IMAGES_URL}/logo_wbg_small2.png{else}{$BASE_URL}/send_file.php?object_type=primary_company_logo{/if}"
                    alt="{$ORGANIZATION_NAME}"></a></div>
                ...
                <div id="contentBoxTwo">
                    <div id="rowWarning" valign="center">
                        ...
                        {elseif strtolower($exception) == 'dbinitialize'}
                            {$APPLICATION_NAME} {t}database has not been initialized yet, please run the installer again and follow the on screen instructions.{/t}
                            <a href="{$BASE_URL}/install/install.php?external_installer=1">{t}Click here to run the installer now.{/t}</a>
                        {else}

{if $section == 'header'}
	<tr>
		<td colspan="{$total_columns}" align="left">
			<ul id="tabmenu">
				<li><img style="vertical-align: top" id="tab_hide_img" src="{$IMAGES_URL}/nav_bottom_sm.gif" onClick="FAIRNESS.searchForm.toggleTabBlock()"></li>
				<li><a id="tab_basic_search" href="javascript:FAIRNESS.searchForm.showTab('basic_search'); {$tab_onclick}">{t}Basic Search{/t}</a></li>
				<li><a id="tab_adv_search" href="javascript:FAIRNESS.searchForm.showTab('adv_search'); {$tab_onclick}">{t}Advanced Search{/t}</a></li>
				<li><a id="tab_saved_search" href="javascript:FAIRNESS.searchForm.showTab('saved_search')">{t}Saved Search & Layout{/t}</a></li>
...
{elseif $section == 'global'}
	<tr id="tab_global" style="display: none">
		<td colspan="{$total_columns}" class="tblHeader">
			...
			<input type="submit" name="action:clear" value="{t}Clear{/t}" onClick="FAIRNESS.searchForm.clearForm(); selectAll( document.getElementById('filter_column') );">

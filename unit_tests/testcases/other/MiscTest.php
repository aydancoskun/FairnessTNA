...
class MiscTest extends PHPUnit\Framework\TestCase {
	...
	function testEncryptionA() {
		//Make sure we force the salt so its consistent even when the settings.ini.php is not.
		global $config_vars;
	...
	function testDatabaseLoadBalancerSessionVarsB() {
		...
		foreach( $db_hosts as $db_host_arr ) {
			Debug::Text( 'Adding DB Connection...', __FILE__, __LINE__, __METHOD__, 1);
			if(isset($config_vars['database']['persistent_connections'])) $persistent_connections = $config_vars['database']['persistent_connections'];
			else $persistent_connections = false;
			$db_connection_obj = new ADOdbLoadBalancerConnection( $config_vars['database']['type'], $db_host_arr[1], $db_host_arr[2], (bool)$persistent_connections, $db_host_arr[0], $config_vars['database']['user'], $config_vars['database']['password'], $config_vars['database']['database_name'] );
			$db_connection_obj->getADODbObject()->SetFetchMode(ADODB_FETCH_ASSOC);
			$db_connection_obj->getADODbObject()->noBlobs = TRUE; //Optimization to tell ADODB to not bother checking for blobs in any result set.
			$db_connection_obj->getADODbObject()->fmtTimeStamp = "'Y-m-d H:i:s'";
			$db->addConnection( $db_connection_obj );
	...
	function testRemoteHTTP() {
		...
		$this->assertEquals( (int)$header_size, -1 ); //30 Bytes.

		$header_size = (int)Misc::getRemoteHTTPFileSize('https://'.$url);
		$this->assertEquals( (int)$header_size, 400 ); //30 Bytes.
		...
		$this->assertEquals( (int)$size, 400 ); //30 Bytes.
	...
	function testIsSubDirectory() {
		...
		$parent_dir = '/var/www/FairnessTNA556688';
		$child_dir = '/var/www/FairnessTNA556688Test';
		$this->assertEquals( Misc::isSubDirectory( $child_dir, $parent_dir ), FALSE );
		...
		$parent_dir = '/var/www/FairnessTNA556688Test';
		$child_dir = '/var/www/FairnessTNA556688';
		$this->assertEquals( Misc::isSubDirectory( $child_dir, $parent_dir ), FALSE );

		$parent_dir = '/var/www/FairnessTNA556688';
		$child_dir = '/var/www/FairnessTNA556688/storage';
		$this->assertEquals( Misc::isSubDirectory( $child_dir, $parent_dir ), TRUE );
	...
	function testSOAPClient() {
		$this->assertEquals( Misc::ping(), TRUE );
	}

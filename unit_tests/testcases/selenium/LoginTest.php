...
class UILoginTest extends PHPUnit_Extensions_Selenium2TestCase
{
    ...
    public function testUILoginLogout()
    {
        $this->Login();
        $this->Logout();
    }
...
//		$this->waitForAttribute( 'css=div.login-view@init_complete' );
    ...
    public function waitForAttribute($attribute_name, $value = 'true', $timeout = false)
    {
        if ($timeout == '') {
    ...
    public function waitForElementPresent($attribute_name, $value = true, $timeout = false)
    {
        if ($timeout == '') {
            $timeout = $this->selenium_config['default_timeout'];
        }

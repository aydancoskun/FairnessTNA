...
<div id="rowContentInner">
		<form method="post" name="report" action="{$smarty.server.SCRIPT_NAME}">
		    ...
		    <div id="contentBoxTwoEdit">
				...
				{if !isset($setup_data.p1_1_psea_ids)
					OR !isset($setup_data.p1_2_psea_ids)}
					<tr class="tblDataError">
						<td colspan="3">
				...
				<tr>
					<td colspan="2" class="{isvalid object="ugdf" label="tax" value="cellLeftEditTable"}">
						{t}State where you pay contributions:{/t}
					</td>
					<td class="cellRightEditTable">
						<select id="columns" name="setup_data[state_id]">
							{html_options options=$setup_data.state_options selected=$setup_data.state_id|default:$current_company->getProvince()}
						</select>
					</td>
				</tr>
				...
				<tr>
					<td colspan="2" class="{isvalid object="ugdf" label="tax" value="cellLeftEditTable"}">
						{t}State Reporting Number:{/t}
					</td>
					<td class="cellRightEditTable">
						<input type="text" name="setup_data[state_report_number]" size="20" value="{$setup_data.state_report_number}">
					</td>
				</tr>

				<tr>
					<td colspan="2" class="{isvalid object="ugdf" label="tax" value="cellLeftEditTable"}">
						{t}Total Payments (Line 1):{/t}
					</td>
					<td class="cellRightEditTable">
						<select id="columns" name="setup_data[p1_1_psea_ids][]" size="{select_size array=$filter_data.income_pay_stub_entry_account_options}" multiple>
							{html_options options=$filter_data.income_pay_stub_entry_account_options selected=$setup_data.p1_1_psea_ids}
						</select>
					</td>
				</tr>
				...
				<tr>
					<td colspan="2" class="{isvalid object="ugdf" label="tax" value="cellLeftEditTable"}">
						{t}Exempt Payments (Line 2):{/t}
					</td>
					<td class="cellRightEditTable">
						<select id="columns" name="setup_data[p1_2_psea_ids][]" size="{select_size array=$filter_data.deduction_pay_stub_entry_account_options}" multiple>
							{html_options options=$filter_data.deduction_pay_stub_entry_account_options selected=$setup_data.p1_2_psea_ids}
						</select>
					</td>
				</tr>
				</tbody>
			...
			<div id="contentBoxFour">
				...
				{* <input type="submit" name="BUTTON" value="{t}Display Report{/t}" onClick="this.form.target = '_blank'; document.getElementById('action').name = 'action:Display Report';"> *}
			</div>

...
class PayStubTransactionFactory extends Factory {
	...
	function postSave() {
		...
		if ( $this->getDeleted() == FALSE AND is_object($rs_obj) AND $rs_obj->getType() == 3000 AND $rs_obj->getDataFormat() == 5 AND in_array( $this->getStatus(), array(100, 200) ) ) { //3000=EFT/ACH, 5=FairnessTNA EFT, 100=Stop Payment, 200=Stop Payment (ReIssue)
			Debug::Text( '  Issuing Stop Payment for FairnessTNA PaymentServices... ', __FILE__, __LINE__, __METHOD__, 10 );

			//Send data to FairnessTNA Remittances service.
			...
			foreach ( $pstlf_sorted_array as $pstlf_sub_sorted_array ) {
				...
				foreach ( $pstlf_sub_sorted_array as $pst_obj ) {
					...
					if ( $pst_obj->getStatus() == 10 ) {
						...
						//FairnessTNA PaymentServices API loop
						if ( ( $export_type == 'export_transactions' ) AND $rs_obj->getType() == 3000 AND $rs_obj->getDataFormat() == 5 ) { //3000=EFT/ACH 5=FairnessTNA Payment Services
							//START BATCH
							if ( $n == 0 ) {
								//Send data to FairnessTNA Remittances service.
								$tt_ps_api = $le_obj->getPaymentServicesAPIObject();
						...
						} //end FairnessTNA Remittances API loop
					...
					foreach ( $praelf as $prae_obj ) {
						...
						if ( is_array($event_data) AND isset($event_data['flags']) AND $event_data['flags']['auto_pay'] == TRUE ) {
							...
							if ( is_object( $prae_obj->getPayrollRemittanceAgencyObject() ) ) {
								...
								if ( $rs_obj->getType() == 3000 AND $rs_obj->getDataFormat() == 5 ) { //3000=EFT/ACH, 5=FairnessTNA EFT  -- This is the remittance source account assigned the remittance agency, not the individual transactions.
														...
														if ( is_array( $output_data ) ) {
															if ( PRODUCTION == TRUE AND is_object( $le_obj ) AND $le_obj->getPaymentServicesStatus() == 10 AND $le_obj->getPaymentServicesUserName() != '' AND $le_obj->getPaymentServicesAPIKey() != '' ) { //10=Enabled
																try {
																	...
																	Debug::Arr( $retval, 'PaymentServices Retval: ', __FILE__, __LINE__, __METHOD__, 10 );
									...
									Debug::Text( '  ERROR! Remittance Source Account is not EFT or FairnessTNA Payment Services!', __FILE__, __LINE__, __METHOD__, 10 );
								}
							} else {

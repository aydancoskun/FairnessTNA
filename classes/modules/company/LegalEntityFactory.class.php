...
//					'-1300-tag' => TTi18n::gettext('Tags'),
	...
	function getPaymentServicesAPIObject() {
		require_once( Environment::getBasePath() . DIRECTORY_SEPARATOR .'classes'. DIRECTORY_SEPARATOR .'modules'. DIRECTORY_SEPARATOR . 'other' . DIRECTORY_SEPARATOR . 'PaymentServices.class.php' );
		$tt_ps_api = new PaymentServices( $this->getPaymentServicesUserName(), $this->getPaymentServicesAPIKey() ); //Username and API Key
			...
			if ( $this->getPaymentServicesUserName() == '' OR $this->getPaymentServicesAPIKey() == '' ) {
				...
				if ( isset( $current_user ) AND is_object( $current_user ) ) {
					try {
						...
						$tt_ps_api = new PaymentServices(); //No API key yet, as we need to create one still.

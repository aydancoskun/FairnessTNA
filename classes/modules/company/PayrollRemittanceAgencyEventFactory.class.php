...
//
			...
			if ( in_array( $this->getFrequency(), array(2000) ) ) { //20=Annual
				// Month
			...
			if ( in_array( $this->getFrequency(), array(3000) ) ) { //30=Quarterly
				// Quarter month
			...
			// Day of week
			if ( in_array( $this->getFrequency(), array(5100) ) ) { //90=Weekly
			...
			if ( in_array( $this->getFrequency(), array(1000) ) ) { //10=Pay Period
		...
		if ( $this->getDeleted() != TRUE ) {
			if ( $this->getStatus() == 15 ) { //15=Enabled - Full Service
				...
				if ( is_array($event_data) AND isset($event_data['flags']) AND ( $event_data['flags']['auto_pay'] == TRUE ) ) {
					...
					//Make sure the source account is of FairnessTNA Payment Services format.
					if ( is_object( $this->getPayrollRemittanceAgencyObject() )
							AND is_object( $this->getPayrollRemittanceAgencyObject()->getRemittanceSourceAccountObject() )
							AND $this->getPayrollRemittanceAgencyObject()->getRemittanceSourceAccountObject()->getDataFormat() != 5 ) { //5=FairnessTNA Payment Services
						$this->Validator->isTrue( 'status_id',
												  FALSE,
	...
	function preSave() {
		...
		if ( $this->isNew( TRUE ) OR $this->getEnableRecalculateDates() == TRUE ) {
			...
			if ( isset( $due_date_array['start_date'] ) AND isset( $due_date_array['end_date'] ) AND isset( $due_date_array['due_date'] ) ) {
				...
				if ( $include_columns == NULL OR ( isset($include_columns[$variable]) AND $include_columns[$variable] == TRUE ) ) {
					...
					switch( $variable ) {
						...
						case 'type':
							...
							if ( method_exists( $this, $function ) ) {
								$data[$variable] = Option::getByKey( $this->$function(), $this->getOptions( $variable, array( 'payroll_remittance_agency_id' => $this->getPayrollRemittanceAgencyId() ) ) );
							}
							break;
						case 'status':
							$function = 'get'.$variable;
		...
		switch ($frequency_type_id) {
			...
			case 2200: //semi-annually
				...
				if ( $last_due_date <= $primary_date ) {
					...
					$due_date = TTDate::incrementDate( $end_date, $this->getDueDateDelayDays(), 'day' );
				} elseif ( $last_due_date <= $secondary_date ) {
					//later this year
			...
			case 4200: //semi-monthly
				...
				} elseif ( $day_of_month <= $secondary_day_of_month ) {
					//late this month
...
//				break;
			...
			case 60000: //US - Monthly (Quarter Exceptions)
				...
				$day = 15;
				$exception_months = array(3, 6, 9, 12); //March, June, September and December
			...
			case 90100: //Upon Hire
				$retval = array( //By default clear out these dates.
								 'due_date'   => '',
								 'start_date' => '',
								 'end_date'   => '',
				);
				...
				if ( TTDate::getMiddleDayEpoch( $last_due_date ) == TTDate::getMiddleDayEpoch( $current_epoch ) OR ( $this->getDueDate() == '' OR TTDate::getMiddleDayEpoch( $this->getDueDate() ) == TTDate::getMiddleDayEpoch( $this->getLastDueDate() ) ) ) {
				...
				if ( is_object( $this->getCompanyObject() ) ) {
					...
					//As of schema version 1093A, the hire date column was changed to a date_stamp rather than epoch, which causes a SQL error during upgrade from old versions of FairnessTNA.
			...
			case 90200: //Upon Termination
				...
				if ( is_object( $this->getCompanyObject() ) ) {
					...
					if ( $ulf->getRecordCount() > 0 ) {
						...
						foreach( $ulf as $u_obj ) {
							if ( $earliest_termination_date == FALSE OR $u_obj->getTerminationDate() < $earliest_termination_date ) {
		...
		if ( isset($retval['start_date']) AND $retval['start_date'] != '' AND isset($retval['end_date']) AND $retval['end_date'] != '' AND isset($retval['due_date']) AND $retval['due_date'] != '' ) {
			...
			return $retval;
		} elseif ( isset($retval['start_date']) AND $retval['start_date'] == '' AND isset($retval['end_date']) AND $retval['end_date'] == '' AND isset($retval['due_date']) AND $retval['due_date'] == '' ) {
	...
	function getRecurringHolidayDates( $start_epoch = FALSE ) {
		...
		if ( is_object($pra_obj) ) {
			...
			if ( $start_epoch != FALSE AND is_array( $selected_holidays ) AND count( $selected_holidays ) > 0 ) {
	...
	function getURL( $action_id ) {
		...
		$url = 'https://github.com/aydancoskun/FairnessTNA';//'?agency_id='. $agency_id .'&event_type_id='. $event_type_id .'&action_id='. $action_id;
	...
	function getReport( $report_id, $data, $user_obj, $permission_obj ) {
		...
		if ( isset( $report_obj_name ) AND $report_obj_name != ''
			...
			$report_data['config']['legal_entity_id'] = array($this->getPayrollRemittanceAgencyObject()->getLegalEntity());

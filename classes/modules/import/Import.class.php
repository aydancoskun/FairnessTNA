		...
		if ( is_array( $raw_data_columns ) AND is_array($columns) ) {
			...
			foreach( $raw_data_columns as $raw_data_key => $raw_data_column ) {
				...
				if ( $matched_column_key !== FALSE AND isset( $columns[ $matched_column_key ] ) ) {
					if ( in_array( $matched_column_key, $matched_columns) !== FALSE ) { //Once a file column is matched to a FairnessTNA field, don't match it again.
...
//					//Only if the two columns don't match though, as there was a bug that if someone tried to import column names that matched the FairnessTNA
